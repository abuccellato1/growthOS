...
export async function POST(request: Request) {
  ...
  let body: {
    ...
    rating?: number
    feedbackText?: string
    emailFeedbackItems?: EmailFeedbackItem[]
  }
  try { body = await request.json() } catch { return apiError('Invalid body', 400, 'INVALID_BODY') }

  const { businessId, outputId, rating, feedbackText, emailFeedbackItems } = body
  ...
  if (emailFeedbackItems && emailFeedbackItems.length > 0) {
    const rows = emailFeedbackItems.map(item => ({
      business_id: businessId,
      deliverable_type: 'signal_sequences',
      content_block_id: `email_${item.emailNumber}`,
