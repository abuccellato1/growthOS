...
import { Building2, Loader, ExternalLink, RefreshCw, MessageSquare, ArrowRight, CheckCircle, MapPin, Mic } from 'lucide-react'
  ...
  return (
    <div>
      ...
      <input
        ...
        style={{ borderColor: focused ? '#43C6AC' : '#e5e7eb', boxShadow: focused ? '0 0 0 3px rgba(67,198,172,0.12)' : 'none', fontFamily: 'DM Sans, sans-serif' }}
...
function getNextResearchDate(lastResearchAt: string | null): Date | null {
  if (!lastResearchAt) return null
  const last = new Date(lastResearchAt)
  const next = new Date(last)
  next.setDate(next.getDate() + 7)
  return next
}

function canRunResearch(lastResearchAt: string | null, isFirstTime: boolean): boolean {
  if (isFirstTime || !lastResearchAt) return true
  const next = getNextResearchDate(lastResearchAt)
  if (!next) return true
...
export default function BusinessSignalsPage() {
  ...
  const [sessions, setSessions] = useState<Array<{
    id: string; status: string; phase: number; created_at: string; icp_generated_at: string | null
  }>>([])
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: bizData } = await supabase
        .from('businesses').select('*').eq('id', activeBizId).single()
      ...
      const { data: sessionData } = await supabase
        .from('sessions')
        .select('id, status, phase, created_at, icp_generated_at')
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const { data } = await supabase
        .from('businesses')
        .select('research_status, business_research, last_research_at')
        .eq('id', business.id).single()
      if (data?.research_status === 'complete') {
        setBusiness(prev => prev ? { ...prev, research_status: data.research_status, business_research: data.business_research, last_research_at: data.last_research_at } : prev)
  ...
  async function handleSaveProfile() {
    ...
    try {
      ...
      setEditing(false)
      setSuccessMsg('Business profile saved.')
  ...
  async function handleRefreshResearch() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSuccessMsg('Research refresh failed.')
        setTimeout(() => setSuccessMsg(''), 5000)
      }
    } catch {
  ...
  async function handleDeleteBusiness() {
    ...
    try {
      const res = await fetch('/api/businesses/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ businessId: business.id, confirmName: deleteConfirmText }),
      })
      if (res.ok) {
  ...
  const researchRunning = business.research_status === 'running'
  const isFirstResearch = !business.last_research_at
  const researchAllowed = canRunResearch(business.last_research_at, isFirstResearch)
  const nextResearchDate = getNextResearchDate(business.last_research_at)
  ...
  return (
    <div className="max-w-6xl">
      ...
      {/* 1. Place verification banner */}
      {!business.place_id && !showPlaceSearch && (
        <PlaceVerificationBanner onVerifyClick={() => setShowPlaceSearch(true)} />
      )}

      {/* 2. Place search overlay */}
      {showPlaceSearch && (
        <div className="p-6 rounded-2xl border mb-6" style={{ borderColor: '#43C6AC', backgroundColor: 'rgba(67,198,172,0.04)' }}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold" style={{ fontFamily: 'Playfair Display, serif', color: '#191654' }}>Find your business on Google</h3>
            <button onClick={() => setShowPlaceSearch(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af' }}>✕</button>
          </div>
          <BusinessPlaceSearch
            onSelect={async (place: SelectedBusiness) => {
              ...
              try {
                ...
                // Bypass rate limit for place verification
          ...
          />
          {verifying && <p className="text-sm text-center mt-4" style={{ color: '#43C6AC' }}>Verifying and refreshing business data...</p>}
        </div>
      )}

      {/* 3. Research running indicator */}
      {researchRunning && (
        <div className="flex items-center gap-3 p-4 rounded-xl mb-6" style={{ backgroundColor: 'rgba(67,198,172,0.08)', border: '1px solid rgba(67,198,172,0.2)' }}>
          ...
          <p className="text-sm" style={{ color: '#191654' }}>Alex is researching your business profile. Some data may update in a moment.</p>
        </div>
      )}
      ...
      {/* 4. SignalMap Interview CTA — full width */}
      {hasNoSession && (
        <div
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-base" style={{ fontFamily: 'Playfair Display, serif', color: '#191654' }}>Start Your SignalMap Interview</p>
                <p className="text-xs mt-0.5" style={{ color: '#6b7280' }}>Alex is ready. This takes 20-30 minutes and unlocks all your modules.</p>
              </div>
            </div>
      ...
      {/* 5. Two-column grid: Business Profile + Alex Research */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        {/* Business Profile card — left column */}
        <div className="p-6 rounded-2xl border h-fit" style={{ borderColor: '#e5e7eb', backgroundColor: '#ffffff' }}>
          <div className="flex items-center justify-between mb-4">
            ...
            {!editing && (
              <button onClick={openEdit} className="text-sm font-medium" style={{ color: '#43C6AC', background: 'none', border: 'none', cursor: 'pointer' }}>Edit</button>
            )}
          </div>
          ...
          {editing ? (
            <div className="space-y-4">
              <div className="mb-2">
                {business.place_id ? (
                  <div className="flex items-center gap-2 p-3 rounded-xl" style={{ backgroundColor: 'rgba(67,198,172,0.08)' }}>
                    ...
                    <p className="text-xs" style={{ color: '#43C6AC' }}>Google verified</p>
                    <button type="button" onClick={() => { setEditing(false); setShowPlaceSearch(true) }} className="text-xs ml-auto" style={{ color: '#9ca3af', background: 'none', border: 'none', cursor: 'pointer' }}>Change →</button>
                  </div>
                ) : (
                  <button type="button" onClick={() => { setEditing(false); setShowPlaceSearch(true) }} className="w-full flex items-center justify-center gap-2 p-3 rounded-xl border text-sm font-semibold" style={{ borderColor: '#43C6AC', color: '#43C6AC', backgroundColor: 'rgba(67,198,172,0.04)' }}>
                    <MapPin size={16} /> Verify Your Business on Google
                  </button>
                )}
              </div>
              ...
              <EditInput label="Google My Business Profile URL" value={gmbUrl} onChange={setGmbUrl} placeholder="e.g. https://g.page/your-business" hint="Your GMB profile URL" required={false} />
              ...
              {researchAllowed ? (
                <button onClick={handleSaveAndRefresh} disabled={saving || researchRunning} className="w-full py-3 rounded-xl font-semibold text-white text-sm disabled:opacity-60" style={{ backgroundColor: '#43C6AC', fontFamily: 'DM Sans, sans-serif' }}>
                  {saving ? 'Saving and refreshing research...' : 'Save & Refresh Alex\'s Research'}
                </button>
              ) : (
                <div>
                  <button onClick={handleSaveProfile} disabled={saving} className="w-full py-3 rounded-xl font-semibold text-white text-sm disabled:opacity-60 mb-2" style={{ backgroundColor: '#43C6AC', fontFamily: 'DM Sans, sans-serif' }}>
                    {saving ? 'Saving...' : 'Save Changes'}
                  </button>
                  <p className="text-xs text-center" style={{ color: '#9ca3af' }}>
                    Research refresh available {nextResearchDate ? nextResearchDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric' }) : 'soon'}
                  </p>
                </div>
              )}
              <button onClick={() => { setEditing(false); setSuccessMsg('') }} disabled={saving} className="w-full text-center text-sm py-1" style={{ color: '#9ca3af', background: 'none', border: 'none', cursor: 'pointer' }}>Cancel</button>
            </div>
          ) : (
        ...
        <div className="p-6 rounded-2xl border h-fit" style={{ borderColor: '#e5e7eb', backgroundColor: '#ffffff' }}>
          ...
          {research ? (
            <>
              <p className="text-xs mb-4" style={{ color: '#9ca3af' }}>Last scanned: {business.last_research_at ? formatDate(business.last_research_at) : formatDate(business.updated_at)}</p>
            ...
            <p className="text-sm" style={{ color: '#9ca3af' }}>No research data yet. Save your business profile to start research.</p>
          )}
        </div>
      </div>

      {/* 6. CustomerSignals — full width */}
            ...
            {((vocDetail.top_phrases as string[]) || []).length > 0 && (
              <div className="flex flex-wrap gap-2 mb-4">
                {((vocDetail.top_phrases as string[]) || []).slice(0, 6).map((phrase: string, i: number) => (
                  <span key={i} className="text-xs px-3 py-1.5 rounded-full font-medium" style={{ backgroundColor: 'rgba(67,198,172,0.12)', color: '#43C6AC' }}>&ldquo;{phrase}&rdquo;</span>
                ))}
              </div>
            )}
              ...
              <div className="mb-4">
                ...
                <ul className="space-y-1">
                  {((vocDetail.outcome_language as string[]) || []).slice(0, 3).map((item: string, i: number) => (
                    <li key={i} className="text-xs flex items-start gap-2" style={{ color: '#6b7280' }}><span style={{ color: '#43C6AC' }}>→</span> {item}</li>
                  ))}
                </ul>
              </div>
            )}
            <Link href="/dashboard/voice-of-customer" className="text-xs font-medium" style={{ color: '#43C6AC' }}>Add more to CustomerSignals →</Link>
          </>
        ) : (
          <div className="text-center py-4">
            <p className="text-sm mb-1 font-medium" style={{ color: '#374151' }}>No customer signals yet</p>
            <p className="text-xs mb-4" style={{ color: '#9ca3af' }}>Run business research or add reviews manually to extract customer language for your modules.</p>
            <Link href="/dashboard/voice-of-customer" className="text-xs font-semibold" style={{ color: '#43C6AC' }}>Add to CustomerSignals →</Link>
          </div>
        )}
      </div>
      ...
      <div className="p-6 rounded-2xl border mb-6" style={{ borderColor: '#e5e7eb', backgroundColor: '#ffffff' }}>
        ...
        {sessions.length === 0 ? (
          ...
          <div className="space-y-3">
            {sessions.map((s) => (
              <div key={s.id} className="flex items-center justify-between py-2 border-b last:border-0" style={{ borderColor: '#f3f4f6' }}>
                <div className="flex items-center gap-3">
                  <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: s.status === 'completed' ? '#43C6AC' : s.status === 'in_progress' ? '#f59e0b' : '#d1d5db' }} />
                  <div>
                    <p className="text-sm font-medium" style={{ color: '#191654' }}>
                      {s.status === 'completed' ? 'SignalMap Interview complete' : s.status === 'in_progress' ? `Interview in progress (Phase ${s.phase})` : 'Interview not started'}
                    </p>
      ...
      <div className="mt-8 p-6 rounded-2xl border-2" style={{ borderColor: '#fca5a5', backgroundColor: '#fff5f5' }}>
        ...
        <div className="flex items-center justify-between p-4 rounded-xl border" style={{ borderColor: '#fca5a5', backgroundColor: '#ffffff' }}>
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: '#9ca3af' }}>Your business data is archived for 30 days, then permanently deleted.</p>
          </div>
          <button onClick={() => setShowDeleteModal(true)} className="px-4 py-2 rounded-lg text-sm font-semibold text-white flex-shrink-0 ml-4" style={{ backgroundColor: '#ef4444' }}>Delete Business</button>
        </div>
      </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)' }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
            <h3 className="text-xl font-bold mb-2" style={{ fontFamily: 'Playfair Display, serif', color: '#191654' }}>Delete {business?.business_name}?</h3>
            <p className="text-sm mb-4" style={{ color: '#6b7280' }}>This will archive your business and all associated data for 30 days. After 30 days everything is permanently deleted.</p>
            <div className="p-3 rounded-lg mb-4" style={{ backgroundColor: '#fff5f5', border: '1px solid #fca5a5' }}>
              ...
              <ul className="text-xs space-y-1" style={{ color: '#6b7280' }}>
                ...
                <li>CustomerSignals data</li>
                <li>Signal Score history</li>
              </ul>
            </div>
            <p className="text-sm mb-2" style={{ color: '#374151' }}>Type <strong>{business?.business_name}</strong> to confirm:</p>
            <input type="text" value={deleteConfirmText} onChange={(e) => setDeleteConfirmText(e.target.value)} placeholder={business?.business_name} className="w-full px-3 py-2.5 rounded-lg border text-sm outline-none mb-4" style={{ borderColor: '#e5e7eb' }} />
            <div className="flex gap-3">
              <button onClick={() => { setShowDeleteModal(false); setDeleteConfirmText('') }} className="flex-1 py-2.5 rounded-xl text-sm font-semibold" style={{ backgroundColor: '#f3f4f6', color: '#374151' }}>Cancel</button>
              <button onClick={handleDeleteBusiness} disabled={deleteConfirmText !== business?.business_name || deleting} className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white disabled:opacity-40" style={{ backgroundColor: '#ef4444' }}>{deleting ? 'Deleting...' : 'Delete Business'}</button>
            </div>
          </div>
        </div>
      )}
