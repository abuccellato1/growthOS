...
function UnsplashImage({
  ...
  useEffect(() => {
    fetchUnsplashImage(query + ' professional service').then(url => setImageUrl(url))
  }, [query])
...
function ReelScriptCard({
  ...
  return (
    <div className="p-4 rounded-xl border" style={{ borderColor: '#e5e7eb' }}>
      ...
      <UnsplashImage query={reel.pillar + ' ' + unsplashQuery} height={140} />
      ...
      <div className="space-y-3 mt-3">
        ...
        <div className="flex items-start justify-between gap-2">
          <p className="text-xs" style={{ color: '#6b7280' }}>
            <strong>Caption:</strong> {reel.captionSuggestion}
          </p>
  ...
  return (
    <div className="p-4 rounded-xl border" style={{ borderColor: '#e5e7eb' }}>
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="text-xs font-bold" style={{ color: '#191654' }}>
            Carousel {index + 1}: {carousel.pillar}
          </p>
      ...
      <UnsplashImage query={carousel.pillar + ' ' + unsplashQuery} height={140} />
      ...
      <div className="space-y-2 mt-3">
        ...
        >
          <p className="text-xs font-bold mb-0.5" style={{ color: '#43C6AC' }}>
            COVER SLIDE
          </p>
          <p className="text-xs font-semibold" style={{ color: '#191654' }}>
            {carousel.coverSlide.headline}
          </p>
          <p className="text-xs" style={{ color: '#6b7280' }}>
            {carousel.coverSlide.subtext}
          </p>
        ...
        {carousel.slides.map((slide, si) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold mb-0.5" style={{ color: '#191654' }}>
                {slide.headline}
              </p>
        ...
        <div className="p-3 rounded-lg" style={{ backgroundColor: '#191654' }}>
          ...
          <p className="text-xs" style={{ color: '#ffffff' }}>
            {carousel.closingSlide.text}
          </p>
          <p className="text-xs font-semibold mt-1" style={{ color: '#43C6AC' }}>
            {carousel.closingSlide.cta}
          </p>
        </div>
      </div>
  ...
  return (
    <div className="p-4 rounded-xl border" style={{ borderColor: '#e5e7eb' }}>
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="text-xs font-bold" style={{ color: '#191654' }}>
            Story {index + 1}: {story.pillar}
          </p>
          <p className="text-xs" style={{ color: '#9ca3af' }}>
            {story.frameCount} frames
          </p>
        </div>
        <CopyButton text={fullStory} variant="button" label="Copy sequence" />
      </div>

      <UnsplashImage query={story.pillar + ' ' + unsplashQuery} height={120} />

      <div className="flex gap-2 overflow-x-auto pb-2 mt-3">
        {story.frames.map((frame, fi) => (
          <div
            key={fi}
            className="flex-shrink-0 w-28 rounded-xl p-3 flex flex-col justify-between"
            style={{ backgroundColor: '#191654', minHeight: 150 }}
          >
            ...
            <div>
              ...
              {frame.stickerSuggestion && (
                <p className="text-xs mt-1" style={{ color: '#43C6AC' }}>
                  🎯 {frame.stickerSuggestion}
                </p>
              )}
            </div>
...
export default function BonusFormats({
  reelScripts,
  carouselFrameworks,
  storySequences,
  unsplashQuery = 'professional service business',
}: Props) {
  return (
